  ...
  return (
    <AdminLayout>
      <div className="flex justify-between items-center mb-6">
        <div><h1 className="text-2xl font-heading font-bold">Gestione Staff</h1></div>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><Label>Categoria</Label>
                <select name="category" defaultValue={editing?.category || "board"} className="w-full border rounded-md px-3 py-2 text-sm">
      ...
      {Object.entries(categories).map(([key, label]) => (
        <div key={key} className="mb-6">
          ...
          <div className="space-y-2">
            {staff?.filter((s) => s.category === key).map((s) => (
              <Card key={s.id}>
